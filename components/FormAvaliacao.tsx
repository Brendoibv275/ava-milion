...
import { MOTIVOS_CONTATO, SUGESTOES_AULA } from "@/lib/utils";
...
type TipoSuporte = "SISTEMA" | "MENTORIA" | "AULA";
...
export default function FormAvaliacao({ tipo, titulo, subtitulo, cor }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/avaliacoes", {
                ...
                body: JSON.stringify({ ...form, tipo, nota }),
            });
    ...
    const notaColor =
        notaDisplay >= 8 ? "#22c55e" : notaDisplay >= 5 ? "#f59e0b" : "#ef4444";
        ...
        <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12" style={{ background: "oklch(0.08 0 0)" }}>
            ...
            <form onSubmit={handleSubmit} className="w-full max-w-2xl space-y-5 animate-fade-in">
                ...
                <div className="glass rounded-2xl p-6 space-y-3">
                    <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-widest">Motivo do contato *</h2>
                ...
                {/* Problema Resolvido */}
                <div className="glass rounded-2xl p-6 space-y-3">
                    <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-widest">Seu problema foi resolvido? *</h2>
                ...
                {/* Atendente */}
                <div className="glass rounded-2xl p-6 space-y-3">
                    <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-widest">Atendente *</h2>
                    {atendentes.length === 0 ? (
                        <p className="text-gray-500 text-sm">Carregando atendentes...</p>
                    ) : (
